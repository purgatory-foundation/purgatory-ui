...
function OperatorFunctions (props: any) {
    ...
    const account = props.account !== undefined ? props.account : "";
    const router = useRouter();
    const { data: signer } = useSigner();

    const myApprovals = useGetApprovals(account, null);


    async function set2FAApproval(contractAddress:string, operator:string, status:boolean) {
        setOpenModal(false);        
        try{
            setTxStatus(true);
            if(!signer) throw new Error("Signer not found");
            const contract = new ethers.Contract(purgatoryContract, purgatoryABI, signer);
            const tx = await contract.setApprovalForOperatorApproval(account, operator, contractAddress, status);
            setTx(tx);
            setOpenModal(true);
                                            ...
                                            <div className="flex gap-1">
                                                <div className="flex-shrink-0">
                                                    <button
                                                        type="button"
                                                        disabled={txStatus}
                                                        onClick={() => set2FAApproval(approval.Collection, approval.Operator, true)}
                                                ...
                                                <div className="flex-shrink-0">
                                                    <button
                                                        type="button"
                                                        disabled={txStatus}
                                                        onClick={() => set2FAApproval(approval.Collection, approval.Operator, false)}
                                                        className="inline-flex items-center rounded-none border border-gray-300 px-3 py-2 text-xs font-medium leading-4 text-gray-100 shadow-sm hover:bg-gray-50 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                                    >
